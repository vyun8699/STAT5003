...
#predictions <- predict(grid_search, newdata = test_data)

# Match the best model's row
best_row <- which(grid_search$results$alpha == grid_search$bestTune$alpha & 
                 grid_search$results$lambda == grid_search$bestTune$lambda)

# Extract best model's performance
best_model_performance <- grid_search$results[best_row, ]

# Print best model's accuracy
cat("Best model's accuracy:", best_model_performance$Accuracy, "\n")
```
...
# creat params
ps <- makeParamSet(
  makeDiscreteParam("cost", values = c(0.001, 0.01, 0.1, 1, 10)),
  makeDiscreteParam("gamma", values = c(0.0001, 0.001, 0.01, 0.1, 1))
)
...
```{r, message=FALSE, warning=FALSE, results='hold', fig.show='hold'}
#Utilize various feature selection techniques
...
#Use lda for selecting features
select_data <- lda(debt_quality ~ .,data = processed_data.train)
train_data_lda <- as.data.frame(predict(select_data, processed_data.train)$x)
train_data_lda$debt_quality <- processed_data.train$debt_quality
test_data_lda <- as.data.frame(predict(select_data, processed_data.test)$x)
test_data_lda$debt_quality <- processed_data.test$debt_quality
lda.svm.model <-  svm(debt_quality ~ ., data = train_data_lda, kernel = "radial", cost = tuned_params[0], gamma = tuned_params[1])
...
```{r, message=FALSE, warning=FALSE, results='hold', fig.show='hold'}
# Predict on the test data set using pca
lda.svm.predictions <- predict(lda.svm.model, test_data_lda)
confusion_matrix_lda <- confusionMatrix(lda.svm.predictions, reference = test_data_lda$debt_quality)
#confusion_matrix_lda
confusion_matrix_lda$table
cat('The testing set accuracy is: ',confusion_matrix_lda$overall["Accuracy"],'\n')
cat("Precision:",  confusion_matrix_lda$byClass["Precision"], "\n")
cat("Recall:",  confusion_matrix_lda$byClass["Recall"], "\n")
cat("F1 Score:", 2 * (confusion_matrix_lda$byClass["Precision"] * confusion_matrix_lda$byClass["Recall"]) / (confusion_matrix_lda$byClass["Precision"] + confusion_matrix_lda$byClass["Recall"]), "\n")

overall.results.df['accuracy', 'SVM'] <- confusion_matrix_lda$overall["Accuracy"]
overall.results.df['recall', 'SVM'] <- confusion_matrix_lda$byClass["Sensitivity"]
overall.results.df['precision', 'SVM'] <- confusion_matrix_lda$byClass["Precision"]
overall.results.df['specificity', 'SVM'] <- confusion_matrix_lda$byClass["Specificity"]
overall.results.df['balanced.accuracy', 'SVM'] <- (confusion_matrix_lda$byClass["Sensitivity"] + confusion_matrix_lda$byClass["Specificity"]) / 2
