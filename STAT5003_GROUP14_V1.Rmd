...
Steps taken to clean the data are shown below: 

**1. Checking for nulls,blanks, and duplicates:**<br>
...
**4. Extract 'relationship_length' as an additional feature and  'debt_quality' as the target:** <br>
...
+ **Box plot**: variability of total income by occupation type shown below. Any income that is too far from the median may be sign of fraud.
...
ntree_vals <- c(1100) #c(500,700,900,1100,1300)
mtry_vals <- c(8) #c(6,7,8) 
...
#model
rf.best.model <- randomForest(debt_quality ~., data = processed_data.test, ntree=1300, mtry = 8)
