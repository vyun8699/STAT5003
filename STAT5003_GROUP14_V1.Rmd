...
df <- data.frame(
  debt_category = c('0+ days past due',
                    '30+ days past due',
                    '60+ days past due',
                    '90+ days past due',
                    '120+ days past due',
...
bar_plot <- ggplot(df, aes(x = debt_category)) +
  ...
  labs(title = 'Bad debtors (count)', x = 'Category', y = 'Count') +
...
line_plot <- ggplot(df, aes(x = debt_category)) +
  ...
  labs(title = 'Bad debtor (% total)', x = 'Category', y = 'Proportion') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Print the values
cat('Best model parameters (maximizing f1 score):\n')
cat("- ntree: ", ntree_max,"\n")
cat("- mtry: ", mtry_max,"\n")
cat('\n')
...
cat('Best model parameters(maximizing accuracy score):\n')
...
cat('- f1 score: ',LogRes.train_results.f1,'\n')
...
cat("Results of the best model on the training set:\n")
cat(paste("Accuracy:", cm_train$overall['Accuracy'], "\n"))
cat(paste("Sensitivity:", cm_train$byClass['Sensitivity'], "\n"))
cat(paste("Precision:", cm_train$byClass['Precision'], "\n"))
cat(paste("Specificity:", cm_train$byClass['Specificity'], "\n"))
cat(paste("Balanced Accuracy:", (cm_train$byClass['Sensitivity'] + cm_train$byClass['Specificity']) / 2, "\n"))
cat(paste("F1 Score:", 2*(cm_train$byClass['Precision'] * cm_train$byClass['Sensitivity'])/((cm_train$byClass['Precision'] + cm_train$byClass['Sensitivity']), "\n"))
...
cat('Best model parameters (maximizing balanced accuracy):\n')
...
cat('- balanced accuracy: ', max_balanced_accuracy, "\n")
cat('- f1 score: ', max_f1, "\n")
...
cat('Best model parameters (maximizing f1 score):\n')
cat("- class weights: ", class_weight, "\n")

cat("Training scores for the best model:\n")
cat('- accuracy: ', best_result$accuracy , "\n")
cat('- precision: ', best_result$precision, "\n")
cat('- sensitivity: ', best_result$sensitivity, "\n")
cat('- specificity: ', best_result$specificity, "\n")
cat('- balanced accuracy: ', best_result$balanced.accuracy, "\n")
cat('- f1 score: ', best_result$f1.score, "\n")
