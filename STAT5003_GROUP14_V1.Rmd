...
```{r, warnings=FALSE, fig.show='hold',results='hold'}
...
# Train the LDA model with grid search
...
test_predictions <- predict(lda_model, newdata = processed_data.test)
test_conf_mat <- confusionMatrix(test_predictions, processed_data.test$debt_quality)

# Calculate and print the test metrics
test_acc <- test_conf_mat$overall['Accuracy']
test_precision <- test_conf_mat$byClass['Pos Pred Value']
test_sensitivity <- test_conf_mat$byClass['Sensitivity']
test_specificity <- test_conf_mat$byClass['Specificity']
test_f1 <- 2 * test_conf_mat$byClass['Sensitivity'] * test_conf_mat$byClass['Specificity'] / 
  (test_conf_mat$byClass['Sensitivity'] + test_conf_mat$byClass['Specificity'])



# Update overall results dataframe for the test dataset
overall.results.df['accuracy','LDA'] <- test_acc
overall.results.df['precision','LDA'] <- test_precision
overall.results.df['sensitivity','LDA'] <- test_sensitivity
overall.results.df['specificity','LDA'] <- test_specificity
overall.results.df['balanced.accuracy','LDA'] <-(test_sensitivity + test_specificity)/2
overall.results.df['f1.score','LDA'] <- test_f1
```
...
We recommend Random Forest (RF) for this analysis. although it still has its limitations, as shown by its specificity score (0.58, caused by moderately high false positives). This implies that the RF model is 'too strict' whereby good debtors are flagged as bad debtors. This limitation may be caused by data imbalance which has been minimized by SMOTE but still presents bias. We are comfortable with this flaw as having false positives carry lesser economic impact than having false negatives (e.g., lost revenue instead of bad debt on the banking book).

To improve model performance, additional data may be needed (e.g. higher volume of customer data and expanding features collected at application). Other useful features that may be collected include information on wealth level, history of indebtedness at other financial institutions, criminal records,  etc.
