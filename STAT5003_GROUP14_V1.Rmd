...
+ **Initial data analysis and cleaning:** (i) the dataset was screened for nulls, blanks, and duplicates, (ii) trimmed to only include relevant IDs with 20+ months banking history, (iii) and seperated into two classes with label 'class 1' for customers with 30+ days due, and 'class 0' for everyone else. **The resulting dataset of 21,575 customers contains 2,895(c.13%) customers in class 1**.
...
+ Cumulative spread of bad debtors shown below. 88.9% of debtors has more than 0 days of deliquency which could very well be an honest mistake and not representative of their ability to service their payments. Thus we defined 30+ days of deliquency as our target feature.
...
+ Initial and resulting correlation plots shown below. Correlation between features can be seen by fewer red boxes.
...
max_accuracy <- rf.results.df$accuracy[max_row_index]
max_precision <- rf.results.df$precision[max_row_index]
max_f1 <- rf.results.df$f1.score[max_row_index]
...
cat("- mtry: ", mtry_max,"\n")
cat('\n')
cat("Other training scores for the best model:\n")
...
overall.results.df['accuracy','AdaBoost'] <- best_model_params$accuracy
overall.results.df['precision','AdaBoost'] <- best_model_params$precision
overall.results.df['sensitivity','AdaBoost'] <- best_model_params$sensitivity
overall.results.df['specificity','AdaBoost'] <- best_model_params$specificity
overall.results.df['balanced.accuracy','AdaBoost'] <- best_model_params$balanced.accuracy
overall.results.df['f1.score','AdaBoost'] <- best_model_params$f1.score
...
rounded.results.df
barplot(rounded.results.df$Value, names.arg = rounded.results.df$Category, xlab = "Categories", ylab = "Values", col = "blue", main = "Bar Plot of Values")
```

**We measure model performance through precision and sensitivity:**
+ **precision** (TP / (TP+FP)): a high precision score implies that the model performs well in differentiating true positive against false positives (e.g., out of all customers predicted as bad debtors, how many are really bad debtors?).
+ **sensitivity** (TP/(TP+FN)): a high sensitivity score implies that the model performs well in differentiating true positive against fall negatives (e.g., out of all actual bad debtors, how many were actually flagged as bad debtors by the model?). 
